...
{
    QString ret = QString("shirecoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SCN unit input in lnReqAmount, we generate the URI with SCN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SCN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
